...
var telegram = originalTelegram
var systemTitle = originalTelegram[2:10]
var frameCounter = originalTelegram[14:18]
var payload = originalTelegram[18 : len(originalTelegram)-12]
var gcmTag = originalTelegram[len(originalTelegram)-12:]


// Test if the state machine in Smarty.go splits the byte stream in the required tokens.
func TestTokenization(t *testing.T) {
    tokenIV, tokenCipher := smarty.ProcessTelegram(telegram[:])

    iv := append(systemTitle, frameCounter...)
    cipher := append(payload, gcmTag...)

    ivBool := bytes.Equal(tokenIV, iv)
    cipherBool := bytes.Equal(tokenCipher, cipher)

    if ivBool && cipherBool {
        t.Logf("Tokenization success: \n")
    } else {
        t.Errorf("Tokenization failed!\n" +
            "IV: %t\n" +
            "Cipher: %t\n",
            ivBool, cipherBool)
    }
